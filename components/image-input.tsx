...
export function ImageInput({ onResult }: ImageInputProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 1) If we have a local file → OCR + claims + image verify
      if (imageFile) {
        ...
        if (hasText) {
          ...
          claims = await extractClaims(extractedText, (msg) =>
            setProgress(msg)
          );
        } else {
          console.log(
      ...
      // 2) Image authenticity (file OR URL)
      ...
      try {
        const res = await fetch("/api/ingest/image-verify", {
          ...
          body: JSON.stringify({
            // If user provided a URL, use that;
            // otherwise send base64 of uploaded file (preview)
        ...
        if (res.ok) {
          ...
          console.warn(
            "[ImageInput] /api/ingest/image-verify failed:",
            await res.text()
          );
        }
      } catch (err) {
      ...
      // 3) Build result – note: for URL-only case, claims will be [] and raw_text = ""
      ...
      toast({
        ...
        description:
          imageFile && extractedText.trim()
            ? `Extracted ${claims.length} claim(s) and checked image authenticity`
            : "Checked image authenticity from the provided URL",
      });
    } catch (error) {
